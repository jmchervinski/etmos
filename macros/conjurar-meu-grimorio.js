/**
 * ═══════════════════════════════════════════════════════════════
 * ETMOS · Conjurar Frase Mágica — Meu Grimório (para jogadores)
 * ───────────────────────────────────────────────────────────────
 * Versão da macro de conjuração para os JOGADORES: mostra apenas
 * as Partículas que o seu personagem possui em "Meu Grimório"
 * (as marcadas na aba Grimório da ficha). O jogador marca as
 * partículas NA ORDEM em que quer revelar — um selo numerado
 * aparece em cada uma conforme é marcada — e clica em Revelar.
 *
 * O personagem usado é o do token selecionado ou, sem token, o
 * Personagem atribuído ao usuário (Configuração de Usuários).
 *
 ...
 *      pra dentro do mundo e ajustar DECK_NAME abaixo se preciso.
 *   3. Dar aos jogadores permissão de OBSERVER (no mínimo) nesse
...
if (!actor) {
  ui.notifications.warn("Selecione o token do seu personagem, ou peça ao Mestre para atribuir um Personagem ao seu usuário.");
  return;
}
if (actor.type !== "character") {
  ui.notifications.warn(`"${actor.name}" não é um Personagem (Orador). Selecione o token do seu personagem.`);
  return;
}

const config = game.etmos?.config?.particulas;
if (!config) {
  ui.notifications.error("Configuração do sistema Etmos indisponível (game.etmos.config). O sistema está atualizado?");
  return;
}
...
// ---------- Meu Grimório: só as Partículas marcadas na ficha ----------
const grim = actor.system?.grimorio ?? {};
const categorias = [
  ["Funções", config.funcoes],
  ["Objetos", config.objetos],
  ["Características", config.caracteristicas],
  ["Complementos", config.complementos]
];

const porCategoria = {};
const semCarta = [];
let possuidas = 0;
for (const [cat, lista] of categorias) {
  for (const part of lista ?? []) {
    if (grim[part.id] !== true) continue;
    possuidas++;
    const carta = deck.cards.getName(`${part.nome} - ${part.abrev}`);
    if (!carta) { semCarta.push(`${part.nome} - ${part.abrev}`); continue; }
    (porCategoria[cat] ??= []).push(carta);
  }
}

if (!possuidas) {
  ui.notifications.warn(`O Grimório de ${actor.name} está vazio. Marque Partículas na aba Grimório da ficha.`);
  return;
}
if (semCarta.length) {
  console.warn("ETMOS | Partículas do Grimório sem carta no baralho:", semCarta);
  ui.notifications.warn(`${semCarta.length} Partícula(s) do seu Grimório não têm carta no baralho (veja o console). O baralho importado está atualizado?`);
}
if (!Object.keys(porCategoria).length) {
  ui.notifications.error("Nenhuma das suas Partículas foi encontrada no baralho. Reimporte o Baralho de Partículas do compêndio.");
  return;
}

// Layout em TABELA, sem handlers inline (CSP do Foundry) — eventos ligados
// via addEventListener depois que a janela renderiza.
...
const content = `
  <p><b>${actor.name}</b> — selecione as partículas do seu Grimório <b>na ordem</b> em que devem ser reveladas (mínimo ${MIN_PARTICULAS}):</p>
...
const confirmou = await foundry.applications.api.DialogV2.wait({
  window: { title: `🔮 Conjurar Frase Mágica — ${actor.name}`, resizable: true },
